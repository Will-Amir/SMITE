...
%Biomass overtime
% gC/m2 .* m2
% for i = 1:length(gridstate.time_myr)
%     bio_past(i) = sum(nansum(gridstate.BIOMASS_past(:,:,i) .* INTERPSTACK.gridarea .* 1e6 )) ;
%     bio_future(i) = sum(nansum(gridstate.BIOMASS_future(:,:,i).* INTERPSTACK.gridarea .* 1e6 )) ; 
%     bio_tot(i) = sum(nansum(gridstate.BIOMASS_tot(:,:,i))) ; 
% end


% plot(state.time_myr, state.biomass_tot) ;
% xlabel('Time (Ma)')
% ylabel('Sum of biomass (g C/m^{2})')
% title('Biomass over time')


%Difference between the final and penultimate biomass
% figure
...
%     
%     subplot(6,4,i)
% %     land = double(circshift(INTERPSTACK.land(:,:,i),22,2)) ; 
% %     land(land == 0) = NaN;  
% %     m_pcolor(INTERPSTACK.lon,INTERPSTACK.lat,land)
% %     hold on
%     m_pcolor(INTERPSTACK.lon,INTERPSTACK.lat,circshift(log10(gridstate.BIOMASS_tot(:,:,i)),22,2))
%     m_grid('fontsize', 0.1)
%     title(INTERPSTACK.time(i))
%     colorbar
% %     colormap(biomass_colour/246)
% %     hold off
% end
