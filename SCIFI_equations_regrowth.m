function dy = SCIFI_equations_regrowth(t,y)
...
atfrac = atfrac0 * (atmoA/pars.atmoA0) ; % variable
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%% Insolation
ins = (biopars.ins_present - ( biopars.ins_present * 4.6/100 * ( abs( t_geol )/570 ) ));%.*reducemap ;
surfacetemps=nthroot((ins/0.9)/(5.67e-8),4)-273.25;

%%%%%%% Ice (< -10 degC) or no runoff = no biomass 
Tair_ice_past = Tair_past ; 
Tair_ice_future = Tair_future ; 
Tair_ice_past( Tair_ice_past < -5 ) = NaN ; 
Tair_ice_future( Tair_ice_future < -5 ) = NaN ; 
...
biomass_tem_past=biomasstocarry;
biomass_bor_past=biomasstocarry;
biomass_tro_past=biomasstocarry;
biomass_tem_future=biomasstocarry;
biomass_bor_future=biomasstocarry;
...
%%%RESEEDING ALGORITHM%%%
potentialgrowth=find(land_past==1 & currentcull==0);
for count = 1:length(potentialgrowth)
    growthcell=potentialgrowth(count);
    growthcellx=floor(growthcell/biopars.x_lon)+1;
    growthcelly=rem(growthcell,biopars.x_lon);
    if growthcelly==0
        tochecky=[39,40];
        growthcelly=40;
    elseif growthcelly==1
        tochecky=[1,2];
    else
        tochecky=[growthcelly-1,growthcelly,growthcelly+1];
    end
    if growthcellx==1
        tocheckx=[48,1,2];
    elseif growthcellx==48
        tocheckx=[47,48,1];
    elseif growthcellx==49
        tocheckx=[47,48,1];
        growthcellx=48;
    else
        tocheckx=[growthcellx-1,growthcellx,growthcellx+1];
    end
...
%%% Carbon dioxide in total hydrosphere
...
%%% Carbon dioxide in atmosphere only
dy(22)=dy(3)*(atfrac/atfrac0);
